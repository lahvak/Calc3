...
$(x_1,y_1)$ and $(x_2,y_2)$ and whise sides are parallel to the coordinate
axes, the distance between the two points is the length of the diagonal:

```{r, echo=FALSE}
fname <- "../../asymptote/distance/2ddistance.html"
copied <- file.copy(fname, ".", overwrite = TRUE)
htmltools::tags$iframe(title = "Rectangle with corners at the two given points", src = basename(fname), width=385, height=256, frameborder="0")
```
...
In order to fully understand a mathematical idea such as a definition, a theorem or a formula, it is often useful to think about the _special cases_.  In the distance formula, what would happen if the two points have the same $x$-coordinate or $y$-coordinate?  How would the "rectangle" look like in that case?  What would the formula say?  What about _both_ coordinates being the same?
...
evan all three of the coordinates of the two points are equal.  How would the
"box" look like then?)_

```{r, echo=FALSE}
fname <- "../../asymptote/distance/3ddistance.html"
copied <- file.copy(fname, ".", overwrite = TRUE)
htmltools::tags$iframe(title = "Rectangular box with opposite corners at P1 and P2", src = basename(fname), width=285*2, height=256*2, frameborder="0")
```
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/Ld0iOipaKos" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
$\ell^2$ space, or even $L^2$ spaces if you are curious about this.)
