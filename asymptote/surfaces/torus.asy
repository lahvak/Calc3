...
real fx(pair w) {return (3 - cos(w.y))*cos(w.x);}
real fy(pair w) {return (3 - cos(w.y))*sin(w.x);}
real fz(pair w) {return sin(w.y);}
...
//Surface
draw(surface(f, wmin, wmax, nu=60, nv=20, usplinetype = Spline, vsplinetype = new
splinetype[] {notaknot, notaknot, monotonic}),gray+opacity(.8));
