...
real fx(pair w) {return (3+w.y*sin(w.x))*cos(w.x);}
real fy(pair w) {return (3+w.y*sin(w.x))*sin(w.x);}
real fz(pair w) {return w.y*cos(w.x);}

pair wmin = (-pi,-1);
pair wmax = (pi,1);

real [] useq = sequence(mkseq(wmin.x, wmax.x, 18), 18);
