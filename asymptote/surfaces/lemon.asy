...
real fx(pair w) {return cos(w.y)*cos(w.x);}
real fy(pair w) {return cos(w.y)*sin(w.x);}
...
//Surface
draw(surface(f, wmin, wmax, nu=30, usplinetype = Spline, vsplinetype = new
splinetype[] {notaknot, notaknot, monotonic}),yellow+opacity(.8));
